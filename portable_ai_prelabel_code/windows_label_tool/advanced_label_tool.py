...
class ClassDialog(tk.Toplevel):
    ...
    def _refresh(self, _event: object | None = None) -> None:
        ...
        for i, name in enumerate(self.classes):
            row = f"{i:02d}. {ZH.get(name, name)} ({name})"
...
class AdvancedLabelTool:
    ...
    def _load_state(self) -> None:
        ...
        if state.get("labels_dir") and Path(state["labels_dir"]).exists():
            self.labels_dir = Path(state["labels_dir"])
    ...
    def _refresh_box_list(self) -> None:
        ...
        for i, box in enumerate(self.boxes):
            self.box_list.insert(tk.END, f"{i + 1}. {self._class_text(box.class_id)}")
    ...
    def _box_at(self, ix: float, iy: float) -> int | None:
        for i in range(len(self.boxes) - 1, -1, -1):
            x1, y1, x2, y2 = self.boxes[i].to_pixels(*self.current_size)
    ...
    def _drag(self, event: tk.Event) -> None:
        ...
        if self.mode == "resize" and self.anchor_box is not None and self.handle_name is not None:
            x1, y1, x2, y2 = self.anchor_box
            px, py = point
            if "w" in self.handle_name: x1 = px
            if "e" in self.handle_name: x2 = px
            if "n" in self.handle_name: y1 = py
            if "s" in self.handle_name: y2 = py
    ...
    def _render(self) -> None:
        ...
        for i, box in enumerate(self.boxes):
            ...
            self.canvas.create_rectangle(cx1, cy1, cx2, cy2, outline=color, width=3 if i == self.selected else 2)
